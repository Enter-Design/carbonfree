---
title: CarbonFree and Fengate awarded three solar contracts totalling 381 MW in Ontario IESO procurement
dek: Fort Frances, Kynoch and Rainy River will advance toward construction in partnership with the Couchiching, Mississauga and Rainy River First Nations.
date: 2026-04-09
kicker: News · Ontario
category: Press release
image: https://carbonfree.com/wp-content/uploads/2014/11/GRS1.jpg
imageAlt: Utility-scale solar array in Ontario
featured: true
readingTime: 3 min
meta:
  - label: Filed under
    value: Press release
  - label: Procurement
    value: IESO LT2(e1) RFP
awards:
  - capacity: "167.2"
    name: Fort Frances Solar Project
    place: Northwestern Ontario
    partner: In partnership with the Couchiching First Nation
  - capacity: "154"
    name: Kynoch Solar Project
    place: Northeastern Ontario
    partner: In partnership with the Mississauga First Nation
  - capacity: "60"
    name: Rainy River Solar Project
    place: Northwestern Ontario
    partner: In partnership with the Rainy River First Nations
quotes:
  - lead: Meaningful partnerships are foundational to how we develop projects.
    body: We are proud of this outcome and grateful to the IESO for a rigorous and competitive process that evidences how solar-powered, renewable-energy projects can contribute positively to the Ontario energy grid while simultaneously generating carbon free electricity and reducing the cost to Ontario ratepayers. We believe these results reflect our disciplined approach to development and, most importantly, the strength of the partnerships we have built with Indigenous communities across Ontario — and we look forward to continuing this work together as these projects advance toward construction and operation.
    name: Daniel Soper
    role: Chief Executive Officer, CarbonFree
  - lead: This result underscores the strength of our partnership with CarbonFree.
    body: Fengate has a long history of investing in Canadian energy infrastructure, and we are pleased to support the development of these projects as part of Ontario's energy transition. This reflects our shared commitment to delivering high-quality renewable energy assets.
    name: Greg Calhoun
    role: Managing Director, Head of Energy Transition, Fengate Asset Management
contacts:
  - name: Emma Coyle
    org: CarbonFree Technology Inc.
    address: 1 St Clair Ave West, Suite 801, Toronto ON M4V 1K6
    email: emma@carbonfree.com
---


Toronto, Ontario — April 9, 2026 — CarbonFree Technology Inc. (“CarbonFree”), in partnership with Fengate Asset Management (“Fengate”), is pleased to announce that it has been awarded three solar energy long-term supply contracts totalling 381 MW in the Independent Electricity System Operator (“IESO”) Long-Term 2 Energy Supply (Window 1) Request for Proposals (LT2(e1) RFP).

This result represents the largest total contract capacity awarded to a single proponent in this procurement round, and reflects the success of CarbonFree’s long-standing development platform, Fengate’s experience investing in renewable energy infrastructure across North America, and the strong support of our First Nations partners.

## About CarbonFree

CarbonFree Technology Inc. is a 100% Canadian-owned, Ontario-based renewable energy developer with over 20 years of experience developing utility-scale solar and energy infrastructure projects across North America and internationally. [www.carbonfree.com](https://carbonfree.com)
